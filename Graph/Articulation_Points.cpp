...
void dfs(int v, int p = -1)
{
	...
	for (int u : adj[v]) {
		if (!visited[u]) {
			...
			if (low[u] >= tin[v] && p != -1) AP.insert(v); // articulation pt: v //can be repetitive
			++children;
		}
		else if (u != p) {
	...
	if (p == -1 && children > 1) AP.insert(v); // articulation pt: v //can be repetitive
}
...
void solve()
{
	while (1) {
		...
		cout << AP.size() << nl;
		timer = 0, mem(adj), fill(all(visited), 0);
		AP.clear();
	}
}
