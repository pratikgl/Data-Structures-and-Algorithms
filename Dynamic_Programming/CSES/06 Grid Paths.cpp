...
#define int unsigned long long //change int main to int32_t main
...
{
	int n; cin >> n;
	string s[n]; fr0(i, n) cin >> s[i];
	dp[1][1] = 1;
	fr0(i, n) fr0(j, n) {
		if (i == 0 || s[i - 1][j] != '*') (dp[i + 1][j + 1] += dp[i][j + 1]) %= mod;
		if (j == 0 || s[i][j - 1] != '*') (dp[i + 1][j + 1] += dp[i + 1][j]) %= mod;
	}
	if (s[n - 1][n - 1] == '*') cout << 0;
	else cout << dp[n][n];
}
